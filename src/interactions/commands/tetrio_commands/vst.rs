...
impl VstCommand {
    pub async fn from_discord_user(
        ...
        let Some(data) = &tetrio_user.data else {
            return Err(anyhow::anyhow!("❌ Couldn't find user `@{}#{}`", user.resolved.name, user.resolved.discriminator()))
        };
    ...
    pub async fn from_tetrio_user(
        ...
        let Some(data) = &tetrio_user.data else {
            return Ok(Err(anyhow!("❌ No data has been found for user {user}. User might be anonymous or banned.")));
        };

        let (_id, username, Some(apm), Some(pps), Some(vs), rank, tr, Some(glicko), Some(rd)) = (&data.user.id, &data.user.username, data.user.league.apm, data.user.league.pps, data.user.league.vs, &data.user.league.rank, data.user.league.rating, data.user.league.glicko, data.user.league.rd) else {
            return Ok(Err(anyhow::anyhow!("❌ No tetra league stats have been found for user {user}.")));
        };
    ...
    async fn run(
        ...
        match r {
            ...
            Err(_) => {
                interaction_client
                    .update_response(&interaction.token)
                    .content(Some("❌ Message was too long, try lowering the limit."))?
                    .await?
            }
        };
        Ok(Ok(()))
